__author__ = 'cleung'
# Generate reports showing AWS snapshots, AMIs, volumes, and instances; and PURPOSE-tags, ENV-tags
# Snapshots show associated AMIs and PURPOSE-tags thereof
# Volumes show associated instances and the PURPOSE-tags thereof
...
class Resource(object):
    ...
    def get_keep_tag(obj):
        """Get the PURPOSE tag from source, if it exists. Empty strings count as untagged in this version."""
        if 'PURPOSE' in obj.tags and len(obj.tags['PURPOSE'].strip()) != 0:
            return obj.tags['PURPOSE']
        else:
            return "-------no-tag"

    # TODO: will need to support multiple environments beyond PROD and not-PROD
    @staticmethod
    def is_production(obj):
        return 'ENV' in obj.tags and obj.tags['ENV'].upper() == 'PROD'
    ...
    def populate_images(self):
        ...
        for i in images:
            ...
            self.spreadsheet[i.id] = dict(name=i.name, Name_tag=self.get_name_tag(i), id=i.id,
                                          PURPOSE_tag=self.get_keep_tag(i), ENV_tag=self.is_production(i),
    ...
    def populate_volumes(self):
        ...
        for i in volumes:
            ...
            # handle associated instance's PURPOSE-tag
            ...
            if associated_instance_id is None:  # sometimes there is no attached instance
                ...
                instance_keep_tag = Ins.spreadsheet[associated_instance_id]['PURPOSE_tag']
            self.spreadsheet[i.id] = dict(Name_tag=self.get_name_tag(i), id=i.id, PURPOSE_tag=self.get_keep_tag(i),
                                          instance_PURPOSE_tag=instance_keep_tag,
                                          associated_instance_id=associated_instance_id,
                                          ENV_tag=self.is_production(i), attachment_state=i.attachment_state(),
    ...
    def populate_instances(self):
        ...
        Dict is nice so that we can easily look up instance PURPOSE-tags later.
        """
        print "Populating instances info..."
        instances = self.get_all_instances()
        for i in instances:
            self.spreadsheet[i.id] = dict(Name_tag=self.get_name_tag(i), id=i.id, PURPOSE_tag=self.get_keep_tag(i),
                                          ENV_tag=self.is_production(i), instance_type=i.instance_type,
    ...
    def populate_snapshots(self):
        ...
        for i in snapshots:
            ...
            ami_keep_tags = [Ims.spreadsheet[ami_id]['PURPOSE_tag'] for ami_id in associated_ami_ids]

            self.spreadsheet[i.id] = dict(Name_tag=self.get_name_tag(i), id=i.id, PURPOSE_tag=self.get_keep_tag(i),
                                          ami_PURPOSE_tag=ami_keep_tags, associated_ami_ids=associated_ami_ids,
                                          ENV_tag=self.is_production(i), start_time=i.start_time,
...
def generate_volumes_report():
    ...
    list_volumes = sorted(Vols.spreadsheet.values(), key=itemgetter('instance_PURPOSE_tag', 'PURPOSE_tag', 'region',
                                                                    'created'))

    # dump it to see what it looks like
    print "Writing to file..."
    with open(volumes_data_output_file, 'w') as f:
        fields = ['Name', 'volume id', 'volume PURPOSE tag', 'instance PURPOSE tag', 'associated instance id', 'production?',
                  'attachment state', 'volume state', 'status', 'iops', 'size', 'created', 'region']
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for row in list_volumes:
            writer.writerow({'Name': row['Name_tag'], 'volume id': row['id'], 'volume PURPOSE tag': row['PURPOSE_tag'],
                             'instance PURPOSE tag': row['instance_PURPOSE_tag'],
                             'associated instance id': row['associated_instance_id'], 'production?': row['ENV_tag'],
...
def generate_snapshots_report():
    list_snapshots = sorted(Snaps.spreadsheet.values(), key=itemgetter('ami_PURPOSE_tag', 'PURPOSE_tag', 'region',
                                                                       'start_time'))

    # dump it to see what it looks like
    print "Writing to file " + snapshots_data_output_file + "..."
    with open(snapshots_data_output_file, 'w') as f:
        fields = ['Name', 'snapshot id', 'snapshot PURPOSE tag', 'AMI PURPOSE tag', 'associated AMI id', 'production?',
                  'start time', 'region', 'associated volume', 'volume size', 'description']
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for row in list_snapshots:

            # deal with none, single, or multi AMIs and their respective PURPOSE-tags, if existent
            associated_ami_ids = ""
            ami_keep_tags = ""
            if len(row['associated_ami_ids']) == 1:
                associated_ami_ids = row['associated_ami_ids'][0]
                ami_keep_tags = row['ami_PURPOSE_tag'][0]
                ...
                for keeps in row['ami_PURPOSE_tag']:
                    ami_keep_tags += keeps + " "

            writer.writerow({'Name': row['Name_tag'], 'snapshot id': row['id'], 'snapshot PURPOSE tag': row['PURPOSE_tag'],
                             'AMI PURPOSE tag': ami_keep_tags, 'associated AMI id': associated_ami_ids,
                             'production?': row['ENV_tag'], 'start time': row['start_time'], 'region': row['region'],
                             'associated volume': row['associated_volume'], 'volume size': row['volume_size'],
                             'description': row['description']})


def generate_instances_report():
    list_instances = sorted(Ins.spreadsheet.values(), key=itemgetter('PURPOSE_tag', 'region', 'launched'))

    # dump it to see what it looks like
    print "Writing to file " + instances_data_output_file + "..."
    with open(instances_data_output_file, 'w') as f:
        fields = ['Name', 'instance id', 'PURPOSE tag', 'production?', 'instance type', 'state', 'launched', 'region']
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for row in list_instances:
            writer.writerow({'Name': row['Name_tag'], 'instance id': row['id'], 'PURPOSE tag': row['PURPOSE_tag'],
                             'production?': row['ENV_tag'], 'instance type': row['instance_type'],
                             'state': row['state'], 'launched': row['launched'], 'region': row['region']})



def generate_images_report():
    list_images = sorted(Ims.spreadsheet.values(), key=itemgetter('PURPOSE_tag', 'region', 'created'))

    # dump it to see what it looks like
    print "Writing to file " + images_data_output_file + "..."
    with open(images_data_output_file, 'w') as f:
        fields = ['name', 'alternative name', 'image id', 'PURPOSE tag', 'production?', 'region', 'created',
        ...
        for row in list_images:
            ...
            writer.writerow({'name': row['name'], 'alternative name': row['Name_tag'], 'image id': row['id'],
                             'PURPOSE tag': row['PURPOSE_tag'], 'production?': row['ENV_tag'], 'region': row['region'],
                             'created': row['created'],
